...
public class Zombie : LivingEntity
{
    ...
    // 추적할 대상이 존재하는지 알려주는 포로퍼티
    private bool hasTarget
    // 프로퍼티
    {

        get
        { // 외부에서 hasTarget 을 볼떄 실행
    ...
    private IEnumerator UpdatePath()
    {
        ...
        while (!dead)
        {
            ...
            else
            {
                ...
                //모든 콜라이더를 순회하면서 살아 있는 LivingEntitu 찾기
                for (int i = 0; i < colliders.Length; i++)
                {
                    //콜라이더로부터 LivingEntity 컴포넌트 가져오기
                    LivingEntity livingEntity = colliders[i].GetComponent<LivingEntity>();

                    //LivingEnity 컴포넌트가 존재하며, 해당 LivingEntity 가 살아 있다면
    ...
    public override void Die()
    {
        ...
        Collider[] zomibieColliders = GetComponents<Collider>();
        // 콜라이더 2개를 한번에 끄기 위해 복수로 받아옴
        for(int i = 0; i<zomibieColliders.Length;i++)
        {
            zomibieColliders[i].enabled=false;
        }
        ...
        //사망 효과음 재생
        zombieAudioPlayer.PlayOneShot(deaathSound);
    }
